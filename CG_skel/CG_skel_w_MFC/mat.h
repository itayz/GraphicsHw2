...
class mat2 {
	...
	vec2  _m[2]; //each row in mat = vector
...
public:
	...
	friend std::ostream& operator << (std::ostream& os, const mat2& m) //mat transpose is needed
	{
...
public:
	...
	vec4 operator * (const vec4& v) const {  // m * v
		return vec4(_m[0].x * v.x + _m[0].y * v.y + _m[0].z * v.z + _m[0].w * v.w,
			_m[1].x * v.x + _m[1].y * v.y + _m[1].z * v.z + _m[1].w * v.w,
			_m[2].x * v.x + _m[2].y * v.y + _m[2].z * v.z + _m[2].w * v.w,
			_m[3].x * v.x + _m[3].y * v.y + _m[3].z * v.z + _m[3].w * v.w
			);
	}
...
inline
mat3 RotateXmat3(const GLfloat theta)
{
	GLfloat angle = (M_PI / 180.0) * theta;
	mat3 c;
	c[2][2] = c[1][1] = cos(angle);
...
inline
mat3 RotateYmat3(const GLfloat theta)
{
	GLfloat angle = (M_PI / 180.0) * theta;
	mat3 c;
	c[2][2] = c[0][0] = cos(angle);
...
inline
mat3 RotateZmat3(const GLfloat theta)
{
	GLfloat angle = (M_PI / 180.0) * theta;
	mat3 c;
	c[0][0] = c[1][1] = cos(angle);
